...
class AmclNode : public AmclCore::Callbacks {
 public:
  AmclNode();
  ~AmclNode();
 ...
 private:
  ...
  // Particle filter
  AmclCore *amcl_;
  void onUpdateOdom(AMCLOdomDataPtr odata) override;
  void onUpdateLaser(AMCLLaserDataPtr ldata) override;
  void onUpdateParticles(SampleSet *set) override;
  // ParticleFilter *pf_;
  double pf_err_, pf_z_;
  bool pf_init_;
  //Pose pf_odom_pose_;
  double d_thresh__, a_thresh__;
  int resample_interval__;
  //int resample_count_;
...
AmclNode::AmclNode()
  ...
  private_nh_.param("update_min_d", d_thresh__, 0.2);
  private_nh_.param("update_min_a", a_thresh__, M_PI / 6.0);
  private_nh_.param("odom_frame_id", odom_frame_id_, std::string("odom"));
  private_nh_.param("base_frame_id", base_frame_id_, std::string("base_link"));
  private_nh_.param("global_frame_id", global_frame_id_, std::string("map"));
  private_nh_.param("resample_interval", resample_interval__, 2);
...
void AmclNode::onUpdateLaser(AMCLLaserDataPtr ldata) {}
void AmclNode::onUpdateOdom(AMCLOdomDataPtr odata) {}
void AmclNode::onUpdateParticles(SampleSet *set) {
  TRACE_FUNC_ENTER
  ...
  for (int hyp_count = 0;
       hyp_count < set->cluster_count; hyp_count++) {
    double weight;
    Pose pose_mean;
    Covariance pose_cov;
    if (!set->getClusterStats(hyp_count, &weight, &pose_mean, &pose_cov)) {
  ...
  if (max_weight > 0.0) {
    ...
    p.header.stamp = ros::Time::now(); //laser_scan->header.stamp;
    // Copy in the pose
    ...
    try {
      ...
      tf::Stamped<tf::Pose> tmp_tf_stamped(
          tmp_tf.inverse(), ros::Time::now(), base_frame_id_);
    ...
    if (tf_broadcast_ == true) {
      ...
      ros::Time transform_expiration =
          (ros::Time::now() + transform_tolerance_);
...
void AmclNode::reconfigureCB(AMCLConfig &config, uint32_t level) {
  ...
  d_thresh__ = config.update_min_d;
  a_thresh__ = config.update_min_a;

  resample_interval__ = config.resample_interval;
...
void AmclNode::handleMapMessage(const nav_msgs::OccupancyGrid &msg) {
  ...
  amcl_ = new AmclCore(d_thresh__, a_thresh__, resample_interval__);
  amcl_->registerCallbacks(this);
...
*/
  ...
  */
  ...
  // pf_->init(pf_init_pose_mean, pf_init_pose_cov);
  // pf_init_ = false;
  amcl_->initParticleFilter(min_particles_, max_particles_,
                            0.001, //alpha_slow_,
                            0.1, //alpha_fast_,
                            pf_err_, pf_z_, pf_init_pose_mean,
                            pf_init_pose_cov);
  ...
  odom_->SetModel(odom_model_type_, alpha1_, alpha2_, alpha3_, alpha4_,
                  alpha5_);
  */
  amcl_->setOdomModel(odom_model_type_);
  amcl_->initOdom(alpha1_, alpha2_, alpha3_, alpha4_, alpha5_);
  ...
  */
  amcl_->setLaserModel(laser_model_);
  amcl_->initLaser(max_beams_, z_hit_, z_short_, z_max_, z_rand_, sigma_hit_,
                   lambda_short_, 0.0, laser_likelihood_max_dist_, do_beamskip_,
                   beam_skip_distance_, beam_skip_threshold_,
 ...
 */
void AmclNode::convertMap(const nav_msgs::OccupancyGrid &map_msg) {
  TRACE_FUNC_ENTER

  size_t size_x = map_msg.info.width;
  size_t size_y = map_msg.info.height;
  double scale = map_msg.info.resolution;
  double origin_x =
      map_msg.info.origin.position.x + (size_x / 2) * scale;
  double origin_y =
      map_msg.info.origin.position.y + (size_y / 2) * scale;
  // Convert to player format
  std::vector<int> map_data;
  map_data.resize(size_x * size_y);
  for (int i = 0; i < size_x * size_y; i++) {
    map_data[i] = map_msg.data[i];
  }
  amcl_->initMap(size_x, size_y, scale, origin_x, origin_y, map_data);
  TRACE_FUNC_EXIT
}
...
/*
Pose AmclNode::uniformPoseGenerator(void *arg) {
...
void AmclNode::laserReceived(const sensor_msgs::LaserScanConstPtr &laser_scan) {
  ...
  AMCLLaserPtr laser = amcl_->getLaser(laser_scan->header.frame_id);
  if (!laser) {
    tf::Stamped<tf::Pose> ident(
        tf::Transform(tf::createIdentityQuaternion(), tf::Vector3(0, 0, 0)),
        ros::Time(), laser_scan->header.frame_id);
    ...
    // laser mounting angle gets computed later -> set to 0 here!
    laser_pose_v.v[2] = 0;
    amcl_->registerLaser(laser_scan->header.frame_id, laser_pose_v);
  }
  laser.reset();
  laser = amcl_->getLaser(laser_scan->header.frame_id);
  assert(laser);
  ...
  // If the robot has moved, update the filter
    ...
    odom_->UpdateAction(pf_,
                        std::dynamic_pointer_cast<AMCLSensorData>(odata_ptr));

    // Pose at last filter update
    // this->pf_odom_pose = pose;
  }
  */
  amcl_->pfUpdateOdom(pose);
  ...
  if (lasers_update_[laser_index]) {
    ...
    ldata_ptr->sensor = lasers_[laser_index];

    ldata_ptr->ranges.resize(
        laser_scan->ranges.size()); // ldata_ptr->range_count =
    ...
    if (laser_max_range_ > 0.0) {
      ...
      ldata_ptr->range_max = laser_scan->range_max;
    }
    double range_min;
    ...
    for (int i = 0; i < ldata_ptr->ranges.size();
         i++) // for (int i=0; i<ldata_ptr->range_count; i++)
    {
      ...
      if (laser_scan->ranges[i] <= range_min) {
        ...
        ldata_ptr->ranges[i] =
            ldata_ptr
                ->range_max; // ldata_ptr->ranges[i][0] = ldata_ptr->range_max;
      } else {
        // ldata.ranges[i][0] = laser_scan->ranges[i];
        ldata_ptr->ranges[i] =
            laser_scan->ranges[i]; // ldata_ptr->ranges[i][0] =
                                   // laser_scan->ranges[i];
      }
      // Compute bearing
    ...
    ldata_ptr->angle_max =
        angle_min + (ldata_ptr->ranges.size() - 1) * angle_increment;
    ...
    lasers_[laser_index]->UpdateSensor(
        pf_, std::dynamic_pointer_cast<AMCLSensorData>(ldata_ptr));

    // lasers_[laser_index]->UpdateSensor(pf_,
    // std::make_shared<AMCLSensorData>(ldata));
  ...
  */
  AMCLLaserDataPtr ldata_ptr = std::make_shared<AMCLLaserData>();
  ldata_ptr->sensor = laser;

  ldata_ptr->ranges.resize(laser_scan->ranges.size());
  ...
  ROS_INFO("Laser angles in base frame: min: %.3f inc: %.3f", angle_min, angle_increment);
  ...
  // The AMCLLaserData destructor will free this memory
  ROS_ASSERT(!ldata_ptr->ranges.empty());
  for (int i = 0; i < ldata_ptr->ranges.size(); i++) {
    // amcl doesn't (yet) have a concept of min range.  So we'll map short
    // readings to max range.
    if (laser_scan->ranges[i] <= range_min) {
      ldata_ptr->ranges[i] = ldata_ptr->range_max;
    } else {
      ldata_ptr->ranges[i] = laser_scan->ranges[i];
    }
  }
  // Compute bearing
  ldata_ptr->angle_min = angle_min;
...
void AmclNode::applyInitialPose() {
  ...
  if (initial_pose_hyp_ != NULL) {// && map_ != NULL) {
    ...
    // pf_->init(initial_pose_hyp_->pf_pose_mean,
    // initial_pose_hyp_->pf_pose_cov);
    amcl_->pfSetInitPose(initial_pose_hyp_->pf_pose_mean,
                         initial_pose_hyp_->pf_pose_cov);
    pf_init_ = false;
