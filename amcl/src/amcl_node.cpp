...
#define TRACE_FUNC_ENTER \
  do { \
    std::cout << __FILENAME__ << ":" << __LINE__ << " in " << __func__ << ": [ENTER]" << std::endl; \
  } while (0);

#define TRACE_FUNC_EXIT \
  do { \
    std::cout << __FILENAME__ << ":" << __LINE__ << " in " << __func__ << ": [EXIT]" << std::endl; \
  } while (0);
...
class AmclNode {
 ...
 private:
  ...
  static Pose uniformPoseGenerator(MapPtr arg);
  ...
  bool getOdomPose(tf::Stamped<tf::Pose>& pose, double& x, double& y,
  ...
  ros::ServiceServer nomotion_update_srv_;  // to let amcl update samples
  ...
  HypothesisPtr initial_pose_hyp_;
  ...
  LaserModelType laser_model_type_;
  bool tf_broadcast_;
...
AmclNode::AmclNode()
  ...
  double tmp_tol;
  private_nh_.param("transform_tolerance", tmp_tol, 5.1);
  ...
  laser_scan_filter_ = new tf::MessageFilter<sensor_msgs::LaserScan>(
      *laser_scan_sub_, *tf_, odom_frame_id_, 100);
...
void AmclNode::reconfigureCB(AMCLConfig& config, uint32_t level) {
  ...
  if (config.min_particles > config.max_particles) {
    ...
    ROS_WARN(
        "You've set min_particles to be greater than max particles, this isn't "
        "allowed so they'll be set to be equal.");
  ...
  pf_ = std::make_shared<ParticleFilter>(min_particles_, max_particles_,
                                         alpha_slow_, alpha_fast_,
                                         AmclNode::uniformPoseGenerator, map_);

  pf_err_ = config.kld_err;
  pf_z_ = config.kld_z;
  // pf_->pop_err = pf_err_;
  // pf_->pop_z = pf_z_;
  pf_->setPopulationErr(pf_err_);
  pf_->setPopulationZ(pf_z_);

  // Initialize the filter
  // pf_vector_t pf_init_pose_mean = pf_vector_zero();

  Pose init_pose_mean = Pose();
  init_pose_mean.v[0] = last_published_pose.pose.pose.position.x;
  init_pose_mean.v[1] = last_published_pose.pose.pose.position.y;
  init_pose_mean.v[2] = tf::getYaw(last_published_pose.pose.pose.orientation);
  // matrix_t pf_init_pose_cov = pf_matrix_zero();
  Covariance init_pose_cov = Covariance();
  init_pose_cov.m[0][0] = last_published_pose.pose.covariance[6 * 0 + 0];
  init_pose_cov.m[1][1] = last_published_pose.pose.covariance[6 * 1 + 1];
  init_pose_cov.m[2][2] = last_published_pose.pose.covariance[6 * 5 + 5];
  pf_->initGuassian(init_pose_mean, init_pose_cov);
  pf_init_ = false;
  ...
  odom_.reset();
  // odom_ = new AMCLOdom();
  odom_ = std::make_shared<AMCLOdom>();
  ROS_ASSERT(odom_);
  ...
  laser_ = std::make_shared<AMCLLaser>(
      max_beams_, map_);  // laser_ = new AMCLLaser(max_beams_, map_);
  ROS_ASSERT(laser_);
...
void AmclNode::runFromBag(const std::string& in_bag_fn) {
  ...
  BOOST_FOREACH (rosbag::MessageInstance const msg, view) {
    ...
    if (tf_msg != NULL) {
      ...
      for (size_t ii = 0; ii < tf_msg->transforms.size(); ++ii) {
        tf_->getBuffer().setTransform(tf_msg->transforms[ii],
                                      "rosbag_authority");
      }
      continue;
    }
  ...
  tf::Matrix3x3(tf::Quaternion(q.x, q.y, q.z, q.w))
...
void AmclNode::handleMapMessage(const nav_msgs::OccupancyGrid& msg) {
  ...
  pf_ = std::make_shared<ParticleFilter>(min_particles_, max_particles_,
                                         ...
                                         AmclNode::uniformPoseGenerator, map_);
  TRACE_FUNC
  pf_->setPopulationErr(pf_err_);  // pop_err = pf_err_;
  pf_->setPopulationZ(pf_z_);      // pop_z = pf_z_;

  // Initialize the filter

  updatePoseFromServer();
  Pose pf_init_pose_mean;  // pf_vector_t pf_init_pose_mean = pf_vector_zero();
  pf_init_pose_mean.v[0] = init_pose_[0];
  pf_init_pose_mean.v[1] = init_pose_[1];
  pf_init_pose_mean.v[2] = init_pose_[2];
  Covariance pf_init_pose_cov;  // = pf_matrix_zero();
  pf_init_pose_cov.m[0][0] = init_cov_[0];
  pf_init_pose_cov.m[1][1] = init_cov_[1];
  pf_init_pose_cov.m[2][2] = init_cov_[2];
  pf_->initGuassian(
      pf_init_pose_mean,
      pf_init_pose_cov);  // pf_init(pf_, pf_init_pose_mean, pf_init_pose_cov);
  pf_init_ = false;
  ...
  // Laser
  laser_.reset();                          // delete laser_;
  laser_ = std::make_shared<AMCLLaser>(max_beams_, map_);  // laser_ = new AMCLLaser(max_beams_,
                                           // map_);
  ROS_ASSERT(laser_);
  if (laser_model_type_ == LASER_MODEL_BEAM) {
    laser_->setModelBeam(z_hit_, z_short_, z_max_, z_rand_, sigma_hit_,
...
void AmclNode::freeMapDependentMemory() {
  ...
  // delete odom_;
  odom_.reset();  // odom_ = NULL;
  // delete laser_;
  laser_.reset();  // laser_ = NULL;
  TRACE_FUNC_EXIT
}
 ...
 */
// map_t*
MapPtr AmclNode::convertMap(const nav_msgs::OccupancyGrid& map_msg) {
  TRACE_FUNC_ENTER
  // map_t* map = map_alloc();
  MapPtr map = Map::CreateMap();
  TRACE_FUNC
  ROS_ASSERT(map);
  TRACE_FUNC
  ...
  TRACE_FUNC
  std::cout << "map->size = (" << map->size_x << "," << map->size_y << ")" << std::endl;
  map->cells.resize(map->size_x * map->size_y);
  std::cout << "map->cells.size = " << map->cells.size() << std::endl;
  TRACE_FUNC
  ...
  for (int i = 0; i < map->size_x * map->size_y; i++) {
    ...
    map->cells[i] = std::make_shared<Cell>();
    if (map_msg.data[i] == 0) {
...
bool AmclNode::getOdomPose(tf::Stamped<tf::Pose>& odom_pose, double& x,
                           double& y, double& yaw, const ros::Time& t,
                           const std::string& f) {
  TRACE_FUNC_ENTER
  // Get the robot's pose
  tf::Stamped<tf::Pose> ident(
      tf::Transform(tf::createIdentityQuaternion(), tf::Vector3(0, 0, 0)), t,
      f);
  try {
...
// pf_vector_t
Pose AmclNode::uniformPoseGenerator(MapPtr arg) {
  TRACE_FUNC_ENTER
  MapPtr map(arg);  // map_t* map = (map_t*)arg;
#if NEW_UNIFORM_SAMPLING
  unsigned int rand_index = drand48() * free_space_indices.size();
  std::pair<int, int> free_point = free_space_indices[rand_index];
  // pf_vector_t p;
  Pose p;
  p.v[0] = map->toWorldX(free_point.first);  // MAP_WXGX(map, free_point.first);
  p.v[1] =
      map->toWorldY(free_point.second);  // MAP_WYGY(map, free_point.second);
...
bool AmclNode::globalLocalizationCallback(std_srvs::Empty::Request& req,
  ...
  // (void *)map_);
  pf_->initModel(AmclNode::uniformPoseGenerator, map_);
...
void AmclNode::laserReceived(const sensor_msgs::LaserScanConstPtr& laser_scan) {
  ...
  if (frame_to_laser_.find(laser_scan->header.frame_id) ==
      frame_to_laser_.end()) {
    ROS_DEBUG("Setting up laser %d (frame_id=%s)\n",
              (int)frame_to_laser_.size(), laser_scan->header.frame_id.c_str());
    lasers_.push_back(laser_); // lasers_.push_back(new AMCLLaser(*laser_));
    ...
    Pose laser_pose_v; //pf_vector_t laser_pose_v;
  ...
  Pose delta; // pf_vector_t delta = pf_vector_zero();
  if (pf_init_) {
  ...
  // If the robot has moved, update the filter
    ...
    AMCLOdomDataPtr odata = std::make_shared<AMCLOdomData>(); // AMCLOdomData odata;
    odata->pose = pose;
    // HACK
  ...
  if (lasers_update_[laser_index]) {
    AMCLLaserDataPtr ldata = std::make_shared<AMCLLaserData>(); // AMCLLaserData ldata;
    ...
    for (size_t i = 0; i < laser_scan->ranges.size(); i++) {
      ...
      if (laser_scan->ranges[i] <= range_min) {
        ...
        ldata->ranges[i].range = ldata->range_max;
      } else {
        // ldata.ranges[i][0] = laser_scan->ranges[i];
        ldata->ranges[i].range = laser_scan->ranges[i];
      }
      // Compute bearing
      //ldata.ranges[i][1] = angle_min + (i * angle_increment);
      ldata->ranges[i].angle = angle_min + (i * angle_increment);
    }
    ...
    if (!(++resample_count_ % resample_interval_)) {
      pf_->updateResample(); // pf_update_resample(pf_);
      resampled = true;
    }

    //pf_sample_set_t* set = pf_->sets + pf_->current_set;
    SampleSetPtr set = pf_->getCurrentSet();
    ROS_DEBUG("Num samples: %d\n", (int)set->samples.size());
    ...
    if (!m_force_update) {
      ...
      cloud_msg.poses.resize(set->samples.size());
  ...
  if (resampled || force_publication) {
    ...
    for (int hyp_count = 0;
      ...
      double weight;
      Pose pose_mean; // pf_vector_t pose_mean;
      Covariance pose_cov; // pf_matrix_t pose_cov;
      ...
      hyps[hyp_count].pose_mean = pose_mean;
      hyps[hyp_count].pose_cov = pose_cov;
    ...
    if (max_weight > 0.0) {
      ROS_DEBUG("Max weight pose: %.3f %.3f %.3f",
                hyps[max_weight_hyp].pose_mean.v[0],
                hyps[max_weight_hyp].pose_mean.v[1],
                hyps[max_weight_hyp].pose_mean.v[2]);
      ...
      // Copy in the pose
      p.pose.pose.position.x = hyps[max_weight_hyp].pose_mean.v[0];
      p.pose.pose.position.y = hyps[max_weight_hyp].pose_mean.v[1];
      tf::quaternionTFToMsg(
          tf::createQuaternionFromYaw(hyps[max_weight_hyp].pose_mean.v[2]),
      ...
      try {
        tf::Transform tmp_tf(
            tf::createQuaternionFromYaw(hyps[max_weight_hyp].pose_mean.v[2]),
            tf::Vector3(hyps[max_weight_hyp].pose_mean.v[0],
                        hyps[max_weight_hyp].pose_mean.v[1], 0.0));
        tf::Stamped<tf::Pose> tmp_tf_stamped(
            tmp_tf.inverse(), laser_scan->header.stamp, base_frame_id_);
...
void AmclNode::handleInitialPoseMessage(
  ...
  if (msg.header.frame_id == "") {
    ...
    ROS_WARN(
        "Received initial pose with empty frame_id.  You should always supply "
        "a frame_id.");
  }
  // We only accept initial pose estimates in the global frame, #5148.
    ...
    ROS_WARN(
        "Ignoring initial pose in frame \"%s\"; initial poses must be in the "
        "global frame, \"%s\"",
  ...
  try {
    ...
    tf_->waitForTransform(base_frame_id_, msg.header.stamp, base_frame_id_, now,
                          odom_frame_id_, ros::Duration(0.5));
    tf_->lookupTransform(base_frame_id_, msg.header.stamp, base_frame_id_, now,
  ...
  Covariance pf_init_pose_cov; //pf_matrix_t pf_init_pose_cov = pf_matrix_zero();
  ...
  initial_pose_hyp_.reset(); // delete initial_pose_hyp_;
  initial_pose_hyp_ = std::make_shared<Hypothesis>(); // initial_pose_hyp_ = new amcl_hyp_t();
  initial_pose_hyp_->pose_mean = pf_init_pose_mean;
  initial_pose_hyp_->pose_cov = pf_init_pose_cov;
  applyInitialPose();
  TRACE_FUNC_EXIT
}
...
void AmclNode::applyInitialPose() {
  ...
  if (initial_pose_hyp_ && map_) {
    ...
    pf_->initGuassian(initial_pose_hyp_->pose_mean, initial_pose_hyp_->pose_cov);
    pf_init_ = false;
