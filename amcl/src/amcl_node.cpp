...
void AmclNode::onUpdateParticles(SampleSet *set) {
  ...
  if (max_weight > 0.0) {
    ...
    try {
      ...
      tf::Stamped<tf::Pose> tmp_tf_stamped(tmp_tf.inverse(), ros::Time::now(),
                                           base_frame_id_);
...
void AmclNode::convertMap(const nav_msgs::OccupancyGrid &map_msg) {
  ...
  double origin_x = map_msg.info.origin.position.x + (size_x / 2) * scale;
  double origin_y = map_msg.info.origin.position.y + (size_y / 2) * scale;
...
void AmclNode::laserReceived(const sensor_msgs::LaserScanConstPtr &laser_scan) {
  ...
  ROS_INFO("Laser angles in base frame: min: %.3f inc: %.3f", angle_min,
           angle_increment);
